...
function OffreGrid (){
    ...
    return (
        <div>
            <h2 className="text-center">Les sports  </h2>
                    ...
                    {sport.map((items)=>{return(<div className="sport-tit"  key={items.id}><h3 className="police-titre display-6 text-body-emphasis lh-1 mb-3 text-pop-up-top  sport-titre">{items.Sport}</h3>
                         ...
                         <div className="container text-center">
                            <div className=" row g-4 row-cols-3 row-cols-sm-1 row-cols-md-3">
                             { offre.filter((offres)=>offres.SPORT_ID == items.id).map((offres)=>{return(<div className="col " key={offres.id}> <Cardoffre1 offre={offres.Offre} description={offres.Place_offre} prix={offres.Prix_offre}/></div>)})}
                            </div>
                         </div>
